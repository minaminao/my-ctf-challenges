...
想定の倍の参加者数になり、満席状態が続いたり、グッズが不足してしてしまったりと、ごめんなさい。
...
また、最初に目にする問題であもあるため、初心者向けの補足説明もここに書きました。
...
`parseInt` 関数は文字列を整数値に変換しますが、単純に数値を入力しても条件は満たせ無さそうです。
...
ここで42ビットを超える素数 $64527453873583290390233$ と $360296424708927327075211324489217$ は必ずフラグに含まれ、 $3$ と $23$ も確率的にほぼ必ず含まれることがわかります。
...
まず、楕円曲線の位数の性質として、以下の2つがよく知らています:
...
よって先述のパラメータを使用して、この問題を解くことが出来ます:
...
まず、マルチスレッドを独立で実行させるための前準備として、以下のコマンドを実行を実行します:
